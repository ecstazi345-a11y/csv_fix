...
st.title("Паспорт плана месяца / План-Факт")
st.caption("Сопоставление Monthly Passport Plan и агрегированного Daily Progress.")

st.markdown(
    """
## Что здесь делаем

Здесь показывается сопоставление:
- планового объема
- фактического объема
- плановой стоимости
- фактического EV
...
Чтобы видеть, как план месяца соотносится с фактом исполнения.
"""
)
...
try:
    ...
    if df.empty:
        ...
        m2.metric("Фактический объем", f"{total_actual_qty:,.2f}")
        m3.metric("Плановая стоимость", f"{total_plan_value:,.2f}")
        m4.metric("Фактический EV", f"{total_ev:,.2f}")
        ...
        m9, m10 = st.columns(2)
        m9.metric("Direct hours", f"{total_direct_hours:,.2f}")
        m10.metric("Idle hours", f"{total_idle_hours:,.2f}")
